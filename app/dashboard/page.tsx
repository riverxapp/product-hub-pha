...
export default function DashboardPage() {
  ...
  return <div className="flex min-h-screen bg-slate-50 text-slate-950 px-4 py-6 md:px-8 lg:px-12">
  {/* Sidebar Navigation */}
  <nav className="hidden lg:flex lg:flex-col lg:w-64 lg:border-r lg:border-gray-200 bg-white shadow-lg rounded-lg p-6 sticky top-6 self-start h-[calc(100vh-3rem)]">
    <h2 className="sr-only">CRM Navigation</h2>
    <ul className="space-y-2 flex flex-col">
      <li>
        <a href="#" aria-current="page" className="flex items-center px-3 py-2 rounded-md bg-slate-900 text-white font-semibold hover:bg-slate-800">
          Dashboard
        </a>
      </li>
      <li>
        <a href="#" className="flex items-center px-3 py-2 rounded-md text-slate-700 hover:bg-slate-100">
          Customers
        </a>
      </li>
      <li>
  ...
  <main className="flex-1 flex flex-col lg:pl-8">
    {/* Header / Hero */}
    ...
    <div className="flex flex-col lg:flex-row lg:space-x-8">
      ...
      <section className="flex-1 mb-8 lg:mb-0">
        ...
        <input type="search" aria-label="Search records" placeholder="Search by name, company, email or owner" className="w-full px-3 py-2 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 mb-4" value={filter} onChange={(e) => setFilter(e.target.value)} />
        {filteredRecords.length === 0 ? <p className="text-sm text-slate-600">No matching records found.</p> : <div className="overflow-x-auto border rounded-md bg-white shadow-sm">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {filteredRecords.map((record) => <tr key={record.id} className="border-t hover:bg-gray-50">
      ...
      <aside className="w-full max-w-md flex-shrink-0 space-y-8 mt-8 lg:mt-0">
        {/* Recent Activity */}
        <section aria-labelledby="recent-activity-heading" className="bg-white rounded-lg p-6 shadow border border-gray-200">
          ...
          {activity.length === 0 ? <p className="text-sm text-slate-600">No recent activity.</p> : <ul className="space-y-3 text-sm text-slate-700">
              {activity.map((act) => <li key={act.id} className="border-b border-gray-100 last:border-0 pb-2">
                  {act.description}
                </li>)}
            </ul>}
        </section>
        ...
        <section aria-labelledby="pipeline-status-heading" className="bg-white rounded-lg p-6 shadow border border-gray-200">
          ...
          <ul className="space-y-2 text-sm text-slate-700">
            {Object.entries(pipelineSummary).map(([status, count]) => <li key={status} className="flex items-center space-x-3">
                <span className={cn("inline-block w-4 h-4 rounded-full border", badgeClass(status))} />
        ...
        <section aria-labelledby="crm-actions-heading" className="bg-white rounded-lg p-6 shadow border border-gray-200">
          ...
          <div className="flex flex-col space-y-3">
            ...
            {editingId && <Button onClick={() => {resetForm();setIsModalOpen(false);}} variant="outline">
                Cancel Edit
              </Button>}
          </div>
        </section>
    ...
    {/* Modal Form */}
    {isModalOpen && <section role="dialog" aria-modal="true" aria-labelledby="form-heading" tabIndex={-1} onKeyDown={handleDialogKeyDown} className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
        <div className="bg-white rounded-md max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-lg p-6">
          ...
          <form onSubmit={submitRecord} className="space-y-4" noValidate>
            <Field label="Type">
              <select required value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <Field label="Name">
              <input required type="text" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </Field>

            <Field label="Company">
              <input type="text" value={form.company} onChange={(e) => setForm((f) => ({ ...f, company: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </Field>

            <Field label="Email">
              <input type="email" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </Field>

            <Field label="Phone">
              <input type="tel" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </Field>

            <Field label="Status">
              <select required value={form.status} onChange={(e) => setForm((f) => ({ ...f, status: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <Field label="Owner">
              <input type="text" value={form.owner} onChange={(e) => setForm((f) => ({ ...f, owner: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </Field>

            <Field label="Value">
              <input type="number" min={0} value={form.value} onChange={(e) => setForm((f) => ({ ...f, value: Number(e.target.value) }))} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </Field>

            <Field label="Notes">
              <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y" rows={3} />
            </Field>
          ...
          {submitError && <p className="mt-4 text-red-600 font-semibold" role="alert">
              Error: {submitError}
            </p>}
        </div>
      </section>}

    {isModalOpen && <div aria-hidden="true" className="fixed inset-0 z-40 bg-black opacity-30" onClick={closeModal} />}
  </main>
</div>;
...
}: {label: string;value: React.ReactNode;icon: React.ReactNode | string;}) {
  return (
